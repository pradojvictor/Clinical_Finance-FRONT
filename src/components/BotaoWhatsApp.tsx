...
import { WHATSAPP_AGENDAR } from '../config/contato'
import s from './BotaoWhatsApp.module.css'

/**
 * Pop-up flutuante do WhatsApp (canto inferior direito).
 *
 * Fluxo: aparece 10s depois de a página carregar (entrada suave + pulso
 * para chamar o olho). No 1º clique ele ABRE um pouco e mostra "Agendar";
 * com ele aberto, o clique leva ao WhatsApp da clínica (mensagem de
 * agendamento já preenchida). Aberto sem clique por 15s — ou clique em
 * qualquer OUTRO lugar — ele se recolhe de volta ao ícone.
 */
export default function BotaoWhatsApp() {
  ...
  useEffect(() => {
    const timer = setTimeout(() => setVisivel(true), 10_000)
    return () => clearTimeout(timer)
  }, [])

  // Aberto: fecha após 15s sem clicar, ou ao clicar fora do botão.
  useEffect(() => {
    if (!aberto) return
    const timer = setTimeout(() => setAberto(false), 15_000)
  ...
  return (
    <a
      ...
      className={`${s.botao} ${visivel ? s.visivel : ''} ${aberto ? s.aberto : ''}`}
      aria-label={aberto ? 'Agendar consulta pelo WhatsApp' : 'Abrir agendamento pelo WhatsApp'}
      onClick={(e) => {
        // 1º clique só ABRE (mostra o "Agendar"); o clique seguinte navega.
        if (!aberto) {
          e.preventDefault()
          setAberto(true)
        }
